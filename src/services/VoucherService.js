...
class VoucherService {
  ...
  async createVoucher(payload) {
    ...
    return await this.httpClient
      ...
      .post(
        `${this.config.getUrl()}${ApiConstants.ENDPOINTS.VOUCHERS_SLUG}`,
        payload,
      );
  }
  ...
  async createEsimVoucher(payload) {
    ...
    return await this.httpClient
      ...
      .post(
        `${this.config.getUrl()}${ApiConstants.ENDPOINTS.VOUCHERS_ESIM_SLUG}`,
        payload,
      );
  }
  ...
  _validateVoucher(payload) {
    if (!payload.amount || payload.amount === "" || payload.amount < 1) {
      throw new AiraloException(
        `The amount is required, payload: ${JSON.stringify(payload)}`,
      );
    }
    ...
    if (
      ...
      payload.voucher_code.length > 255
    ) {
      throw new AiraloException(
        "The voucher code may not exceed 255 characters.",
      );
    }
    ...
    if (
      ...
      (payload.usage_limit < 1 ||
        payload.usage_limit > SdkConstants.VOUCHER_MAX_NUM)
    ) {
      throw new AiraloException(
    ...
    if (payload.quantity > SdkConstants.VOUCHER_MAX_QUANTITY) {
      throw new AiraloException(
        `The quantity may not be greater than ${SdkConstants.VOUCHER_MAX_QUANTITY}`,
      );
    }
  }
  ...
  _validateEsimVoucher(payload) {
    if (!payload.vouchers || payload.vouchers.length === 0) {
      throw new AiraloException(
        `vouchers field is required, payload: ${JSON.stringify(payload)}`,
      );
    }

    if (!Array.isArray(payload.vouchers)) {
      throw new AiraloException(
        `vouchers field should be an array, payload: ${JSON.stringify(payload)}`,
      );
    }
    ...
    payload.vouchers.forEach((voucher) => {
      if (!voucher.package_id) {
        throw new AiraloException(
          `The vouchers.package_id is required, payload: ${JSON.stringify(payload)}`,
        );
      }

      if (!voucher.quantity) {
        throw new AiraloException(
          `The vouchers.quantity is required and should be greater than 0, payload: ${JSON.stringify(payload)}`,
        );
      }

      if (voucher.quantity > SdkConstants.VOUCHER_MAX_QUANTITY) {
        throw new AiraloException(
          `The vouchers.quantity may not be greater than ${SdkConstants.VOUCHER_MAX_QUANTITY}`,
        );
      }
    });
  }
