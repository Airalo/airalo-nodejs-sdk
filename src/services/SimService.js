...
class SimService {
    ...
    async simUsageBulk(iccids = []) {
        if (!Array.isArray(iccids) || iccids.length === 0) {
            throw new AiraloException('The parameter "iccids" must be an array of ICCID numbers.');
        }
    ...
    getKey(url, params) {
        ...
        return Crypt.md5(`${url}${JSON.stringify(params)}${httpHeaders}${this.accessToken}`);
    }
