...
class Config {
    ...
    constructor(data) {
        ...
        try {
            ...
            throw new AiraloException(`Invalid config data provided, error: ${error.message}`);
        }

        if (!this.data || Object.keys(this.data).length === 0) {
            throw new AiraloException('Invalid config data provided, empty configuration');
        }
        ...
        if (this.data.http_headers !== undefined) {
            ...
            for (const header of this.data.http_headers) {
                if (typeof header !== 'string' || !header.includes(':')) {
                    throw new AiraloException('Invalid HTTP header format. Must be "key: value"');
                }
            }
        }
